                                ...
                                <div class="card-body">
                                    ...
                                    <table id="datatable-buttons" class="table text-center table-bordered dt-responsive nowrap w-100 table-sm">
                                        ...
                                        <tr>
                                            ...
                                            <!-- <td>{{$item->date}}</td> -->
                                            <td>{{$item->name}}</td>
                                            <td><code>{{$item->category->name}}</code></td>
                                            <td>{{$item->status}}</td>
                                            <td>
                                                <a href="{{ url('edit-size/'.$item->id)}} " class="btn btn-primary btn-sm">Edit</a>
                                                |
                                                <a href="{{ url('delete-size/'.$item->id) }}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure to Delete this Record ?')">Delete</a>
                                            </td>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="transaction-detailModalLabel">Size's Details</h5>
            ...
            <div class="modal-body">
                ...
                <div class="table-responsive">
                    <table class="table align-middle table-nowrap">
                        ...
                        <tbody>
                            <form class="needs-validation"  novalidate action="{{ route('store-size')}}"  method="POST">
                                @csrf
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" name="name" class="form-control" id="validationCustom01"
                                                placeholder="Enter Size Number" required>
                                                @if($errors->has('name'))
